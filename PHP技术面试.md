...
* 或者，假设由面试者来设计框架（比如laravel）的数据库、缓存等处理，如何设计
...
* 100个不同的数字分若干组，每组数字相加不超过1000，越接近越好，如何设计？
* 公司有很多场景需要发送消息（邮件，短信、微信模板消息，钉钉模板消息，App通知等等），需要设计一个消息网关，如何设计？考虑哪些方面？
	* 调度策略
	* 鉴权
	* 数据支持
	* 数据持久化
...
* 事务的特性：ACID
* 事务的成功、失败对自增ID的影响
* 事务隔离机制，锁等问题：由低到高依次为`Read uncommitted`、`Read committed`、`Repeatable read`、`Serializable`，这四个级别可以逐个解决脏读、不可重复读、幻读这几类问题。
* 什么是脏读？幻读？不可重复读？
* 什么是死锁？怎么解决？
* 乐观锁和悲观锁分别是什么`
* `in` 和 `exists` 区别？
...
* 实现斐波那契数列的生成
* LeetCode，[第20题，有效的括号](https://leetcode.com/problems/valid-parentheses/)
* 常用的排序算法
